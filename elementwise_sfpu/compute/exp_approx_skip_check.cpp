...
// Unfolded version of exp_tile_init<true, false>() for skip positive check
inline void exp_tile_init_approx_skip_check() {
    // Set constants for exponential calculation with approximation mode and skip positive check
...
// Unfolded version of _calculate_exponential_<true, false, 8, false, true>
inline void _calculate_exponential_approx_skip_check(const int iterations) {
    // This is the unfolded version with the following parameters:
    // APPROXIMATION_MODE = true
    // SCALE_EN = false
    // ITERATIONS = 8
    // FAST_APPROX = false
    // SKIP_POSITIVE_CHECK = true
    
    for (int d = 0; d < iterations; d++) {
        sfpi::vFloat val = sfpi::dst_reg[0];
        
        // APPROXIMATION_MODE is true and SKIP_POSITIVE_CHECK is true
...
// Unfolded version of exp_tile<true, false, true>(0)
inline void exp_tile_approx_skip_check(uint32_t idst) {
...
// DO NOT CALL IT DIRECTLY!!!!
    ...
    llk_math_eltwise_unary_sfpu_params<true>(
        _calculate_exponential_approx_skip_check,
        idst,
        (int)VectorMode::RC,
        8);
#endif
...
void MAIN {
    ...
    exp_tile_init_approx_skip_check();
    ...
    for (uint32_t tile_index = 0; tile_index < actual_tiles_for_this_core; ++tile_index) {
        ...
        // Apply exponent operation with approximation mode and skip positive check
        exp_tile_approx_skip_check(0);

...
// Unfolded version of softplus_tile_init()
inline void softplus_tile_init_unfolded() {
...
// Implementation of softplus function: softplus(x) = log(1 + exp(x))
sfpi_inline sfpi::vFloat _sfpu_softplus_(sfpi::vFloat x)
{
    // For large positive values, softplus(x) ≈ x to avoid overflow
    // For large negative values, softplus(x) ≈ 0
    sfpi::vFloat result;
    
    // For x > 20, return x to avoid overflow in exp
    v_if (x > 20.0f) {
        result = x;
    }
    v_elseif (x < -20.0f) {
        // For very negative values, result is approximately 0
        result = sfpi::vFloat(0.0f);
    }
    v_else {
        // Standard softplus calculation: log(1 + exp(x))
        sfpi::vFloat exp_x = _sfpu_exp_(x);
        sfpi::vFloat one_plus_exp_x = exp_x + sfpi::vFloat(1.0f);
        result = _sfpu_log_(one_plus_exp_x);
    }
    v_endif;
...
// Unfolded version of calculate_softplus
inline void _calculate_softplus_unfolded(const int iterations) {
    for (int d = 0; d < iterations; d++) {
        sfpi::vFloat val = sfpi::dst_reg[0];
        
        // Calculate softplus using the implementation
        sfpi::vFloat result = _sfpu_softplus_(val);
...
inline void softplus_tile_init() {
    softplus_tile_init_unfolded();
}

inline void softplus_tile(uint32_t idst) {

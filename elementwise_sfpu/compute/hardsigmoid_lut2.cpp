...
inline void hardsigmoid_lut2_tile_init_unfolded() {
    // No initialization needed for the LUT-based hardsigmoid
}

// Unfolded version of calculate_hardsigmoid_lut
inline void _calculate_hardsigmoid_lut2_unfolded(const int iterations) {
    // For hardsigmoid: max(0, min(1, (x + 3) / 6))
    
    // Linear coefs (must be pre-scaled because hardsigmoid in scaled space is just identity)
    // TODO : Figure out how to do this with LUT2
    sfpi::vUInt l0 = compile_time_float::encode_coefficients_ct(0.0, 0.0);
    sfpi::vUInt l1 = compile_time_float::encode_coefficients_ct(0.0, 0.0);
    sfpi::vUInt l2 = compile_time_float::encode_coefficients_ct(0.0, 0.0);
    ...
    for (int d = 0; d < iterations; d++) {
        ...
        sfpi::vFloat result = sfpi::lut2(val, l0, l1, l2) * 0;
...
inline void hardsigmoid_lut2_tile_init() {
    hardsigmoid_lut2_tile_init_unfolded();
}

inline void hardsigmoid_lut2_tile(uint32_t idst) {
...
// DO NOT CALL IT DIRECTLY!!!!
    ...
    llk_math_eltwise_unary_sfpu_params<false>(
        _calculate_hardsigmoid_lut2_unfolded,
        idst,
        (int)VectorMode::RC,
        8);
#endif

...
class PrecisionCounter:
    ...
    def _get_quantization_error(self, tensor: torch.Tensor, dtype: torch.dtype) -> torch.Tensor:
        ...
        # Calculate quantization error using quantized/stepped levels
        # This creates constant-height boxes instead of smooth exponential decay
        
        # Find the power-of-2 level for each value
        # For value x, find the largest power of 2 that is <= |x|
        log2_abs = torch.log2(torch.clamp(abs_tensor, min=subnormal_scale))
        power_level = torch.floor(log2_abs)
        
        # Create quantized error levels based on power-of-2 boundaries
        # Each level has constant quantization error = 2^(power_level) * relative_eps
        quantized_level = torch.pow(2.0, power_level)
        quant_error = quantized_level * relative_eps
        
        # Handle very small values (near zero) with minimum error floor
        min_error = subnormal_scale * 10
